...
description: 地层分层字典可视化编辑工具，提供地层结构的手动标定、编辑和数据导出功能。当用户需要处理地层分层数据、创建地层字典或进行地层可视化编辑时使用此技能。支持CSV文件上传、地层边界拖拽调整、实时预览和数据导出。
---
...
#### 步骤1：启动服务
1. 运行 `python scripts/start_server.py`
2. 等待服务启动并自动打开浏览器

#### 步骤2：加载数据
1. 点击"选择文件"按钮上传准备好的地层深度统计CSV文件
2. 点击"加载数据"按钮解析并显示地层结构
...
1. **端口占用**：如果5000端口被占用，修改 `start_server.py` 中的端口号
